...
class SearchHybridNumericHyperparams(hyperparams.Hyperparams):
     ...
     blackbox = hyperparams.Primitive(primitive_families=[PrimitiveFamily.REGRESSION], default=SKRandomForestRegressor, semantic_types=['https://metadata.datadrivendiscovery.org/types/TuningParameter'],description='Black box model to fall back after decision list.')

class SearchHybridNumeric(SupervisedLearnerPrimitiveBase[Input, Output, SearchHybridNumericParams, SearchHybridNumericHyperparams]):
     """
     Class to search for 2-d projection boxes in raw feature space for discrete(categorical) output (for regression problems) .
     ...
     metadata = metadata_base.PrimitiveMetadata({
         ...
         "python_path": "d3m.primitives.cmu.autonlab.find_projections.SearchHybridNumeric",
